import { c as create_ssr_component, d as add_attribute } from './ssr-BDbnMkiA.js';
...
unionType([DispatchEventFrame, OtherEvent, ErrorEvent]);
({
    /**
     * @description
...
objectType({ shortcut: ShortcutKey });
objectType({ skus: arrayType(Sku) });
objectType({ entitlements: arrayType(Entitlement) });
arrayType(Entitlement).nullable();
objectType({
 ...
 */
schemaCommandFactory(Command.AUTHENTICATE);
 ...
 */
schemaCommandFactory(Command.OPEN_SHARE_MOMENT_DIALOG);
 ...
 */
schemaCommandFactory(Command.INITIATE_IMAGE_UPLOAD);

/**
 * Gets all participants connected to the instance
 */
schemaCommandFactory(Command.GET_ACTIVITY_INSTANCE_CONNECTED_PARTICIPANTS);
